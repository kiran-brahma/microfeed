...
describe("podcast uploads", () => {
  ...
  it("uses the existing uploader with a unique key, a permanent URL, and progress", async () => {
    ...
    vi.mocked(Requests.upload).mockImplementation(async (blob: any, key: any, onProgress: any, success: any) => {
  ...
  it("surfaces upload failures without returning a broken URL", async () => {
    vi.mocked(Requests.upload).mockImplementation(async (_blob: any, _key: any, _progress: any, _success: any, failure: any) => { failure(); });
